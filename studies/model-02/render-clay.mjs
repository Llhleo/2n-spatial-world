...
for(const kind of ['A','B','C','C+']){
const model=makeModel(kind[0],kind==='C+');model.updateMatrixWorld(true);
for(const [name,[position,target]] of Object.entries(shots)){
  ...
  model.traverse(mesh=>{if(!mesh.isMesh)return;const g=mesh.geometry,p=g.attributes.position,n=g.attributes.normal,projected=[];
    for(let i=0;i<p.count;i++){const v=new T.Vector3().fromBufferAttribute(p,i).applyMatrix4(mesh.matrixWorld).project(camera);projected.push([(v.x*.5+.5)*width,(.5-v.y*.5)*height,v.z]);}
  ...
  writeFileSync(new URL(`./${kind}-${name}.ppm`,import.meta.url),Buffer.concat([Buffer.from(`P6\n${width} ${height}\n255\n`),pixels]));
}
